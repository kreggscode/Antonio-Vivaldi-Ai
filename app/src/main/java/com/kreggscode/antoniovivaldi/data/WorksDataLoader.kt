package com.kreggscode.antoniovivaldi.data
...
class WorksDataLoader(private val context: Context) {
    ...
    suspend fun loadMajorWorks(): List<MajorWork> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("vivaldi_major_works.json")
                .bufferedReader()
                .use { it.readText() }
    ...
    suspend fun loadEssays(): List<Essay> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("vivaldi_essays.json")
                .bufferedReader()
                .use { it.readText() }
    ...
    suspend fun loadLetters(): List<Letter> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("vivaldi_letters.json")
                .bufferedReader()
                .use { it.readText() }
    ...
    suspend fun loadPapers(): List<Paper> = withContext(Dispatchers.IO) {
        try {
            val json = context.assets.open("vivaldi_papers.json")
                .bufferedReader()
                .use { it.readText() }
